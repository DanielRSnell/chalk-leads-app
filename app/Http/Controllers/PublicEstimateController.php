...
{
    /**
     * Calculate estimate for public widget access (no auth required)
     */
    public function calculate(Request $request, string $widgetKey): JsonResponse
    {
        try {
            // Find widget
            ...
            // Use the exact same logic as the original EstimateController
            ...
            \Log::info('Public estimate calculation responses:', $responses);
            ...
            return response()->json([
                ...
                'estimate' => [
                    ...
                    'total' => round($totalPrice, 2), // Also include new format
                    'base_price' => $basePrice,
                    'subtotal' => round($totalPrice - ($taxAmount ?? 0), 2),
                    'tax_rate' => $taxRate,
            ...
            \Log::error('Public estimate error: ' . $e->getMessage());
    ...
    {
        try {
            // Call the public API endpoint instead
            $controller = new \App\Http\Controllers\Api\WidgetConfigController();
            $response = $controller->show($widget->widget_key);
